...
struct W_DftTransfer::Impl {
  W_DftTransfer *Q = nullptr;
...
W_DftTransfer::W_DftTransfer(int x, int y, int w, int h)
    : W_DftVisu(x, y, w, h),
      P(new Impl) {
  P->Q = this;
...
W_DftTransfer::~W_DftTransfer() {
}

void W_DftTransfer::update_dft_data(
...
VisuDftResolution W_DftTransfer::desired_resolution() const {
  return (P->fdomain == Impl::Domain_Logarithmic) ?
    VisuDftResolution::High : VisuDftResolution::Medium;
}

void W_DftTransfer::reset_data() {
  P->spec.clear();
  P->channels = 0;
}

void W_DftTransfer::draw() {
  //
...
int W_DftTransfer::handle(int event) {
  if (event == FL_ENTER) {
...
void W_DftTransfer::resize(int x, int y, int w, int h) {
  W_DftVisu::resize(x, y, w, h);
  P->reposition_controls();
}

float W_DftTransfer::Impl::frequency_of_x(float x) const {
  return frequency_of_r((x - Q->x()) / Q->w());
}

float W_DftTransfer::Impl::frequency_of_r(float r) const {
...
float W_DftTransfer::Impl::r_of_frequency(float f) const {
  float fmax = 0.5f * fsref;
  switch (fdomain) {
  default:
  case Domain_Linear:
...
float W_DftTransfer::Impl::x_of_frequency(float f) const {
  return Q->x() + r_of_frequency(f) * Q->w();
}

float W_DftTransfer::Impl::nth_frequency_mark(unsigned i) const
{
    switch (fdomain) {
    default:
    case Domain_Linear:
...
int W_DftTransfer::Impl::height_of_mark(unsigned i) const {
    switch (fdomain) {
    default:
    case Domain_Linear: {
...
void W_DftTransfer::Impl::draw_rulers() {
  int x = Q->x();
  int y = Q->y();
  int w = Q->w();
  int h = Q->h();
...
void W_DftTransfer::Impl::draw_back() {
...
void W_DftTransfer::Impl::draw_data() {
  ...
  for (unsigned c = channels; c-- > 1;) {
    const cfloat *ref = &this->spec[0];
    const cfloat *spec = &this->spec[c * specn];

    const unsigned hue = (170 + (c - 1) * 130) % 360;
    ...
    for (int i = 0; i < sw; ++i) {
      ...
      for (unsigned j = 0; j < itp; ++j) {
        cfloat bin = 0.0f;
        if (binidx + j < specn)
          bin = spec[binidx + j] / ref[binidx + j];
        a[j] = std::abs(bin);
      }
...
void W_DftTransfer::Impl::draw_pointer(int mx, int my) {
  int sx = this->screen->x();
  int sy = this->screen->y();
  int sw = this->screen->w();
  int sh = this->screen->h();
...
void W_DftTransfer::Impl::create_controls(bool expanded) {
  int mh = 60;
  int bh = 20;
...
void W_DftTransfer::Impl::reposition_controls() {
  int x = Q->x();
  int y = Q->y();
  int w = Q->w();
  int h = Q->h();
...
void W_DftTransfer::Impl::on_expand_controls() {
  Q->begin();
...
void W_DftTransfer::Impl::on_unexpand_controls() {
  Q->begin();
...
void W_DftTransfer::Impl::changed_fdomain(int val) {
  this->fdomain = val;
  Q->redraw();
}
