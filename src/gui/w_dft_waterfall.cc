...
void W_DftWaterfall::update_data(
  ...
  for (int i = 0; i < w; ++i) {
    ...
    float g = (a > 0.0f) ? (20 * std::log10(a)) : dbneg;
    ...
    color::hsl<float> hcol {170, 50, logdv * 50};
...
void W_DftWaterfall::Impl::draw_pointer(int mx, int my) {
  ...
  fl_line(mx, y+mh, mx, y+h-mh-1);

  char textbuf[32];
  // snprintf(textbuf, sizeof(textbuf), "%g", f);
  sprintf(textbuf, "%d", (int)std::lrint(f));
  textbuf[sizeof(textbuf)-1] = 0;

  fl_font(FL_COURIER, 12);
  int tx = mx+4;
  int ty = my+2;
  int tw = 4+(int)std::ceil(fl_width(textbuf));
  int th = 16;
  int tal = FL_ALIGN_CENTER;
  if (tx + tw > w)
    tx = mx-tw-4;
  ...
  fl_rect(tx, ty, tw, th);
  fl_color(0, 0, 0);
  fl_draw(textbuf, tx, ty, tw, th, tal, nullptr, 0);
}
